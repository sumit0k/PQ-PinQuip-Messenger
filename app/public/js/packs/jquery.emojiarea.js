...
(function ($, window, document) {
	...
	var EmojiArea_WYSIWYG = function ($textarea, options) {
		...
		this.$editor = $('<div>').addClass('tc-white-1');
		this.$editor.text($textarea.val());
		this.$editor.attr({
			'contenteditable': 'true',
			'ng-keypress': 'footer.sendMessage($event)',
			'ng-bind': 'footer.message'
		});
	...
	EmojiMenu.prototype.hide = function () {
		if (this.emojiarea) {
	...
	EmojiMenu.prototype.show = function (emojiarea) {
		if (this.visible || this.emojiarea && this.emojiarea === emojiarea) return;
